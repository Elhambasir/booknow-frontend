...
export const calculateFare = (params: FareCalculationParams): FareBreakdown => {
    ...
    for (const band of distanceBands) {
        const bandDistance = Math.min(remainingDistance, band.limit - previousLimit);
    ...
    const totalFare = subtotal + bookingFee + surgePricing;

    const pricePerMile = distanceInMiles > 0 ? distanceFare / distanceInMiles : distanceBands[0]?.rate ?? 0;
    return {
...
// Generate sample report for comparison with actual taxi services
// export const generateFareReport = () => {
//     const testDistances = [2, 5, 10, 15, 25, 35, 45, 60, 80, 120, 200];
//     const report = testDistances.map(distance => {
//         const estimatedTime = distance * 2.5; // Rough estimate: 2.5 minutes per mile in city traffic
//         const fare = calculateFare({ distanceInMiles: distance, estimatedTimeInMinutes: estimatedTime });
//         return {
//             distance: `${distance} miles`,
//             estimatedTime: `${Math.round(estimatedTime)} min`,
//             totalFare: `£${fare.totalFare}`,
//             avgPerMile: `£${fare.pricePerMile}/mile`,
//             breakdown: `Base: £${fare.baseFare} + Distance: £${fare.distanceFare} + Time: £${fare.timeFare} + Booking: £${fare.bookingFee}`
//         };
//     });
