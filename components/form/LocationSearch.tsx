...
export const LocationSearch: React.FC<LocationSearchProps> = ({
    ...
    try {
      ...
      if (isAirportQuery(term)) {
        ...
        if (airportTerminals.length > 0) {
          ...
          const airportResults: SearchResult[] = airportTerminals.map(terminal => ({
      ...
      const response = await fetch(
        `https://api.simplylookupadmin.co.uk/full_v3/getaddresslist?data_api_key=${API_KEY}&query=${encodeURIComponent(term)}&queryid=1&inputid=2`
      );
  ...
  const getFullAddress = async (id: string): Promise<Location | null> => {
    try {
      const response = await fetch(
        `https://api.simplylookupadmin.co.uk/full_v3/getselectedaddress?data_api_key=${API_KEY}&id=${encodeURIComponent(id)}&inputid=0`
      );
      ...
      if (!data.found || data.error) {
        console.error("Address Error: Could not retrieve full address details.");
        return null;
      }
  ...
  return (
    <div className="relative" ref={searchRef}>
      <div className="relative">
        ...
        {selectedLocation && selectedLocation.latitude !== 0 && !isSearching && (
          <button
            type="button"
          ...
          {searchResults.map((result, index) => (
            <button
              ...
              className={`w-full px-4 py-3 text-left hover:bg-accent border-b border-border last:border-b-0 transition-colors focus:outline-none focus:bg-accent ${
                result.isAirportTerminal ? 'bg-accent/50 border-l-4 border-l-primary' : ''
              }`}
            >
      ...
      {showDropdown && searchResults.length === 0 && !isSearching && searchTerm.trim().length >= 3 && (
        <div className="absolute z-50 w-full mt-1 bg-card border border-border rounded-md shadow-lg p-4 text-sm text-muted-foreground text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">Try searching with a postcode or full address</p>
        </div>
      )}
    </div>
  );
